...
namespace NMediator
{
    public sealed partial class Mediator : IMediator
    {
        ...
        {
            await ProcessMessage(message, matchedHandlerTypes, cancellationToken).ConfigureAwait(false);
        }
        ...
        private async Task<TResponse> SendMessageAsync<TMessage, TResponse>(TMessage message, IEnumerable<Type> matchedHandlerTypes, CancellationToken cancellationToken = default)
            ...
            where TResponse : class, IResponse
        {
            return (TResponse) await ProcessMessage(message, matchedHandlerTypes, cancellationToken).ConfigureAwait(false);
        }

        private async Task<object> ProcessMessage<TMessage>(TMessage message, IEnumerable<Type> matchedHandlerTypes,
            ...
            where TMessage : class, IMessage
        {
            ...
            var context = new MessageContext<TMessage>(message, scope, FindHandlerTypes<TMessage>(matchedHandlerTypes));
